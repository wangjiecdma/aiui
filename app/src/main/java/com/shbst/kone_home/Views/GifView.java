 ...
 * Gifview
 * updata 2018/3/7： 增加回调，监听播放状态，每播放执行一次，则调用一次回调接口
 */
public class GifView extends View {
    ...
    private boolean showLastFrame = false;
    ...
    private GifPlayCallBack mCallBack;
...
//            invalidate();
    ...
    public void lastFrame() {
        ...
        showFirstFrame = false;
        showLastFrame = true;
//        mCurrentAnimationTime = movie.duration() / 2;
        invalidate();
    }
...
//        mCurrentAnimationTime = 0;
    ...
    public void setCallBack(GifPlayCallBack callBack) {
...
//                mCurrentAnimationTime = 400;
    ...
    private void updateAnimationTime() {
        ...
        int countPlayTmp = (int) ((now - mMovieStart) / dur);
        if (countPlay == -1) {
            countPlay = countPlayTmp;
        }
        if (countPlayTmp != countPlay
                && mCallBack != null) {
            countPlay = countPlayTmp;
            mCallBack.playOnceFrameOver(this, dur);
        }
    ...
    public interface GifPlayCallBack {
        void playOnceFrameOver(GifView gifView, int duration);
    }
}